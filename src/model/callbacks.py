...
def get_callbacks(checkpoint_path, patience_lr=10, patience_es=30):
    """
    ...
    # reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=patience_lr, verbose=1, mode='min', min_lr=1e-6)
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_accuracy', factor=0.5, patience=patience_lr, verbose=1, mode='max', min_lr=1e-6)
